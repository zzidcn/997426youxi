 ...
 *
 * 自定义字段：
 *   game997426_url      游戏文件地址（必填，指向 index.html 或单个 .html）
 *   game997426_width    设计宽度（可选，默认 960）
 *   game997426_height   设计高度（可选，默认 640）
 */
get_header();
the_post();

$game_url = esc_url( get_post_meta( get_the_ID(), 'game997426_url', true ) );
$w        = (int) get_post_meta( get_the_ID(), 'game997426_width', true ) ?: 960;
$h        = (int) get_post_meta( get_the_ID(), 'game997426_height', true ) ?: 640;
$ratio    = $h > 0 ? $w / $h : 1.5;
?>
<div class="g99-container">
	<article <?php post_class( 'g99-gamepage' ); ?>>
		...
		<?php else : ?>
			<p class="g99-muted">⚠️ 尚未配置游戏地址（自定义字段 <code>game997426_url</code>）。</p>
		<?php endif; ?>
